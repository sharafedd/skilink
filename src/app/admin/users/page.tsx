...
// Fixed-locale formatters to avoid hydration mismatches
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <form action="/admin/users" method="GET" className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {/* Search */}
            <div className="flex flex-col gap-1 md:flex-row md:items-center md:gap-2">
              <label htmlFor="q" className="text-xs md:text-sm text-muted-foreground md:w-20">
                Search
              </label>
              <input
                id="q"
            ...
            {/* Role */}
            <div className="flex flex-col gap-1 md:flex-row md:items-center md:gap-2">
              <label htmlFor="role" className="text-xs md:text-sm text-muted-foreground md:w-20">
                Role
              </label>
              <select
                id="role"
                name="role"
                defaultValue={role || ""}
                className="w-full rounded-md border px-3 py-2 bg-white text-sm md:text-base"
              >
            ...
            <div className="flex gap-2 md:items-center md:justify-end">
              <Link
                ...
                className="rounded-md border px-3 py-2 text-sm md:text-base"
              >
                Reset
              </Link>
              <button type="submit" className="rounded-md bg-black text-white px-3 py-2 text-sm md:text-base">
                Apply
              </button>
            </div>
          </form>
      ...
      <div className="text-xs sm:text-sm text-muted-foreground">
        Showing <span className="font-medium">{users?.length ?? 0}</span> of <span className="font-medium">{total}</span> users
        {q ? (
          <>
            {" "}for query <span className="font-medium">&ldquo;{q}&rdquo;</span>
          </>
        ) : null}
        {role ? (
          <>
            {" "}with role <span className="font-medium">{role}</span>
          </>
        ) : null}
        .
      </div>
        ...
        <CardContent className="p-0">
          ...
          <div className="sm:hidden divide-y">
            {users && users.length > 0 ? (
              users.map((u: UserRow) => (
                <Link key={u.id} href={`/admin/users/${u.id}`} prefetch={false} className="block p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span className="inline-flex items-center gap-1">
                          Provider: {u.is_provider ? <Badge>Yes</Badge> : <Badge variant="outline">No</Badge>}
                        </span>
                        <span>Created: {dtf.format(new Date(u.created_at))}</span>
                      </div>
                    </div>
                  ...
                  users.map((u: UserRow) => (
                    <TableRow key={u.id}>
                      <TableCell className="truncate">
                        <Link href={`/admin/users/${u.id}`} className="text-blue-600 hover:underline" prefetch={false}>
                          {u.email ?? "(no email)"}
                        </Link>
                      </TableCell>
                      <TableCell>{roleBadge(u.role)}</TableCell>
                      <TableCell>{u.is_provider ? <Badge>Yes</Badge> : <Badge variant="outline">No</Badge>}</TableCell>
                      <TableCell>{df.format(new Date(u.created_at))}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <Link
            href={page > 1 ? `/admin/users${buildQueryString(currentParams, { page: String(page - 1) })}` : "#"}
