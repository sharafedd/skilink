...
function buildQueryString(
  ...
  if (updates.q !== undefined || updates.status !== undefined || updates.budget !== undefined || updates.city !== undefined) {
    merged.set("page", "1");
  }
    ...
    <Card>
      <CardContent className="p-4">
        <form action="/admin/projects" method="GET" className="grid grid-cols-1 gap-3 md:grid-cols-4">
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="status" className="text-sm text-muted-foreground w-20">Status</label>
            <select
              id="status"
              name="status"
              defaultValue={status || ""}
          ...
          <div className="md:col-span-4 flex items-center gap-2 md:justify-end">
            <Link
              href={`/admin/projects${buildQueryString(baseParams, { q: "", status: "", budget: "", city: "", page: "1" })}`}
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2">
        <Link
          href={
            page > 1
              ? `/admin/projects${buildQueryString(baseParams, { page: String(page - 1) })}`
              : "#"
          }
...
export default async function AdminProjectsPage({
  ...
  // Join buyer emails
  const buyerIds = (projects ?? []).map((p: ProjectRow) => p.buyer_id);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="text-sm text-muted-foreground">
        Showing <span className="font-medium">{projects?.length ?? 0}</span> of{" "}
                ...
                (projects as ProjectRow[]).map((p) => {
                  ...
                  return (
                    <TableRow key={p.id}>
                      ...
                      <TableCell><StatusBadge status={p.status as ProjectRow["status"]} /></TableCell>
